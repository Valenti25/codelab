...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, border = false, ...props }, ref) => {
    const Comp = asChild ? Slot : 'button'

    return (
      <Comp
        // ให้ ref เฉพาะตอนเป็น <button>; ถ้าเป็น <Slot> ไม่ส่ง ref
        {...(!asChild ? { ref } : {})}
        data-slot="button"
        className={cn(
          border &&
            'border border-black/10 bg-black/5 duration-200 hover:border-black/20 hover:bg-black/10 dark:border-white/10 dark:bg-white/5 hover:dark:border-white/20 dark:hover:bg-white/10',
